...
export function requireSupabaseSecretConfig(){const{url}=requireSupabasePublicConfig();const secretKey=process.env.SUPABASE_SECRET_KEY;if(!secretKey)throw new Error("Supabase server secret configuration is missing");return{url,secretKey}}
