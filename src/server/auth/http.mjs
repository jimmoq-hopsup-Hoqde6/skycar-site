...
const SAFE_RETURN_ROOTS = [
  { path: '/garage', nested: true },
  { path: '/care/request', nested: false },
  { path: '/care/requests', nested: true },
];
...
function requireSameOrigin(request) {
  if (!isTrustedWriteOrigin(request)) {
    fault('CSRF_FAILED', 403, 'Submit this request from Skycar.');
  }
}
...
export function safeReturnPath(value, fallback = '/garage') {
  ...
  if (parsed.origin !== 'https://skycar.invalid'
    || !SAFE_RETURN_ROOTS.some(root => parsed.pathname === root.path
      || (root.nested && parsed.pathname.startsWith(`${root.path}/`)))) {
    fault('VALIDATION_FAILED', 400, 'Choose a valid Skycar return page.');
  }
