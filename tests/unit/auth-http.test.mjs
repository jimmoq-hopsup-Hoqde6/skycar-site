...
test('same-origin accepts a validated public host when a proxy rewrites the request URL', async () => {
  const { calls, handlers } = setup();
  const proxied = new Request('http://localhost:3100/api/v1/auth/sign-in', {
    method: 'POST',
    headers: {
      origin: 'https://staging.skycar.test',
      host: '127.0.0.1:3100',
      'x-forwarded-host': 'staging.skycar.test',
      'x-forwarded-proto': 'https',
      'content-type': 'application/json',
    },
    body: JSON.stringify({ email: 'person@example.test', password: 'secret', next: '/garage' }),
  });
  const previous = process.env.SKYCAR_APP_ORIGIN;
  try {
    process.env.SKYCAR_APP_ORIGIN = 'https://staging.skycar.test';
    assert.equal((await handlers.signIn(proxied)).status, 200);
    assert.equal(calls.length, 1);
    const forged = request('/api/v1/auth/sign-in', { email: 'person@example.test', password: 'secret' }, {
      origin: 'https://attacker.test', 'x-forwarded-host': 'attacker.test', 'x-forwarded-proto': 'https',
    });
    assert.equal((await handlers.signIn(forged)).status, 403);
    assert.equal(calls.length, 1, 'Forwarded headers do not authorize another sign-in');
  } finally {
    if (previous === undefined) delete process.env.SKYCAR_APP_ORIGIN;
    else process.env.SKYCAR_APP_ORIGIN = previous;
  }
});

test('request-status and My Jobs return paths survive sign-in', () => {
  assert.equal(safeReturnPath('/care/requests/11111111-1111-4111-8111-111111111111'), '/care/requests/11111111-1111-4111-8111-111111111111');
  assert.equal(safeReturnPath('/garage/jobs'), '/garage/jobs');
});
